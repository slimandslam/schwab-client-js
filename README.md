...
2. You need to signup for a [Schwab developer account](https://developer.schwab.com). The account is free. <br />
   ...
   ▪ Use a callback url like `https://127.0.0.1:5556`. Port numbers below 1024 require superuser privileges to "listen" on most systems, so choose a port number above 1024. You can specify up to three callback URLs. Do not use a trailing slash on your callback URL. <br />
...
2. Run `schwab-authorize` OR `manual-authorize`.

- Running the helper app named `schwab-authorize`. This app automatically refreshes your refresh token after you go through the Schwab web page flow that it initiates for you. On MacOS and Linux, you should be able to run it from the command line when you're at the root of your project by typing `schwab-authorize` (for Windows, see Note1 below). It will construct a special Schwab login URL using your SCHWAB_APP_KEY and will try to open your desktop web browser at that URL. You will need to login with your schwab.com credentials (NOT your developer.schwab.com credentials). When you get to the final step, **your browser will likely give you a warning** because `schwab-authorize` dynamically generates a self-signed SSL certificate in order to retrieve the returned HTTPS URL. Allow `schwab-authorize` to access the certificate and it will add the resulting SCHWAB_REFRESH_TOKEN to your .env file. <br />
- Running the helper app named `manual-authorize`. This app requires that you do a few steps manually but you won't have to deal with the self-signed cert warning generated by the `schwab-authorize` app and you'll have more overal control of the process. You can also use no port number or a port number below 1024. The directions given when you run `manual-authorize` should suffice. You'll need to cut and paste the final URL within 30 seconds of its arrival or you'll have to run the app again.

  ▪ Note1: On Windows, you may have to run it directly:  
   `C:\> node node_modules/schwab-client-js/bin/schwab-authorize.js` OR <br />
  `C:\> node node_modules/schwab-client-js/bin/manual-authorize.js` <br />
  ...
  ▪ Note4: `schwab-authorize` is written in Javascript. You can review the source at `node_modules/schwab-client-js/bin/schwab-authorize.js`. The source for `manual-authorize` is at `node_modules/schwab-client-js/bin/manual-authorize.js` The code is also in the github repo for this project. <br />
